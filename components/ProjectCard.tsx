...
const ProjectCard = ({ project, color }: Props) => {
    return (
        <article
            className={`${color} relative flex flex-col rounded-xl h-[260px] md:h-[420px] w-80% md:w-full overflow-hidden group shadow-md`}>
            <div className="flex justify-between px-6 py-10 md:p-14 group-hover:opacity-5">
                <h4 className="text-xl md:text-3xl bg-clip-text text-[#63f3ac]">
                    {project.title}
                </h4>
                <div className="flex gap-2">
                    {project.stacktext.map((stack) => (
                        <div
                            key={stack}
                            className="px-1 lg:px-2 bg-[#4e4d53] rounded-md text-[0.5rem] md:text-xs flex items-center">
                            {stack}
                        </div>
                    ))}
                </div>
            ...
            <div className="opacity-0 hover:opacity-100 duration-300 absolute inset-0 z-10 flex justify-between">
                <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center w-1/2 mx-auto justify-center hover:bg-black/70 ">
                    <svg
